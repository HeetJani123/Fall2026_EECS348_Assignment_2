 ...
 * Collaborators: None.
 * Other sources for code: None. The implementation was authored for this
 *                         assignment; ChatGPT assisted with documentation.
 * Author:      Heet Jani
...
{
    for (;;) {                                 /* Repeat until the replacement is positioned. */
...
static int category_rank(const char *name)
{
    ...
    for (i = 0; i < NUM_CATEGORIES; i++) {     /* Visit each recognized category. */
...
static void do_next(const MaxHeap *h)
{
    ...
    if (!heap_peek(h, &top)) {                  /* Try to inspect the next email. */
...
static void process_stream(FILE *in, MaxHeap *heap)
{
    ...
    while (fgets(line, sizeof(line), in) != NULL) { /* Read until the stream reaches its end. */
        ...
        if (strncmp(text, "EMAIL", 5) == 0 && /* Check for the EMAIL command prefix. */
            ...
            if (parse_email(body, arrival, &e)) { /* Parse and validate the email body. */
                arrival++;                       /* Advance arrival order for the valid email. */
                if (!heap_insert(heap, &e)) {    /* Add the email to the priority heap. */
...
int main(int argc, char *argv[])
{
    ...
    if (argc > 2) {                            /* Accept zero or one optional file argument. */
        fprintf(stderr, "Usage: %s [input-file]\n", argv[0]); /* Show proper program usage. */
        return 1;                              /* Stop because too many arguments were given. */
    }

    if (argc == 2) {                            /* Check whether an input filename was supplied. */
        in = fopen(argv[1], "r");              /* Open the requested file for reading. */
        if (in == NULL) {                       /* Check whether opening the file failed. */
    ...
    if (in != stdin)                            /* Check whether a file was opened. */
